...
CREATE DATABASE omiblog;
CREATE USER 'mromi'@'localhost' IDENTIFIED WITH mysql_native_password BY 'passwordxword10293';
GRANT ALL PRIVILEGES ON *.* TO 'mromi'@'localhost' WITH GRANT OPTION;
ALTER USER 'mromi'@'localhost' REQUIRE NONE WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;
GRANT ALL PRIVILEGES ON `omiblog`.* TO 'mromi'@'localhost';
REVOKE ALL PRIVILEGES ON `omiblog`.* FROM 'mromi'@'localhost'; GRANT ALL PRIVILEGES ON `omiblog`.* TO 'mromi'@'localhost'; ALTER USER 'mromi'@'localhost' ;
